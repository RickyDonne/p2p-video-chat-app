...
function FeaturesStep({ children }: { children: React.ReactElement }) {
  return (
    <div className="flex items-center justify-center space-x-2 font-medium text-purple-500 text-xl ring ring-purple-500 rounded-md px-2 py-2 sm:py-4 w-48 sm:w-60">
      {children}
    </div>
  );
}
  ...
  return (
    <div className="bg-white">
      <Container>
        ...
        <FeaturesHeader text="P2P video chat is more secure than server-based solutions.
         No centralised server can be completely safe from attacks. " />
        <FeaturesContent>
          <>
          End-to-end encryption can enhance security further,
           ensuring that only authorised parties can access the video stream.
          </>
        </FeaturesContent>
        ...
        <FeaturesContent>
          <>
            The only server calls P2P Video Call App makes are to a signalling server that
            helps you find and connect to peers. Room names, participant names
            and, most importantly, video data are never sent to P2P Video Call App. This
        ...
        <FeaturesHeader text="Downside?" />
        <FeaturesContent>
          <>
            P2P Video Call App uses what is referred to as a mesh architecture, where each
